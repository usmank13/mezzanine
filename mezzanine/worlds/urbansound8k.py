...
@dataclass
class UrbanSound8KAdapterConfig:
    csv_path: str
    audio_root: str
    ...
    def validate(self) -> None:
        ...
        audio_root = Path(self.audio_root)
...
def _parse_metadata(csv_path: Path) -> List[Dict[str, Any]]:
    with csv_path.open(newline="", encoding="utf-8") as handle:
        ...
        if reader.fieldnames is None or not required.issubset(set(reader.fieldnames)):
            raise ValueError(f"UrbanSound8K metadata must include {sorted(required)}")
        rows = []
        for row in reader:
            filename = str(row.get("slice_file_name", "")).strip()
            if not filename:
                continue
            rows.append(
                {
...
class UrbanSound8KAdapter(WorldAdapter):
    ...
    def __init__(self, cfg: UrbanSound8KAdapterConfig):
        cfg.validate()
        self.cfg = cfg
    ...
    def _select_rows(self, rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if cfg.classes:
            ...
            rows = [r for r in rows if str(r["class"]).strip().lower() in classes]
    ...
    def _load_item(self, row: Dict[str, Any], label_map: Dict[str, int]) -> Dict[str, Any]:
        ...
        item: Dict[str, Any] = {
            ...
            "sr": int(cfg.sr),
            "label": int(label_map[str(row["class"]).strip()]),
            "meta": {"fold": int(row["fold"]), "class": str(row["class"]).strip()},
        }
    ...
    def load(self) -> Dict[str, Any]:
        ...
        if not rows:
            raise ValueError("No rows matched UrbanSound8K filters.")
